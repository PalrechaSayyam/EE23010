...
Let $Y$ be the gaussian variable,
\begin{align}
	\mu &= np = \frac{n}{2}\\
...
The normal-Distribution of $Z$ is:
\begin{align}
...
\begin{align}
	Q(x) &= \pr{Z>x}\\
	&= \int_{x}^{\infty}\frac{1}{\sqrt{2\pi}}e^{-\frac{t^2}{2}}dt
\end{align}
\begin{enumerate}
\item Without correction
\begin{align}
	Y &\geq 1\\
	Z &\geq \frac{1-\mu}{\sigma}\\
	Z &\geq \frac{2-n}{\sqrt{n}} \label{eq:Z_score-table}
\end{align}
Since we know that,
\begin{align}
	\pr{Z>x} &> 0.9\\
	Q(x) &> 0.9\\
	\implies Z &> 1.28 \label{eq:Zscore}
\end{align}
From \eqref{eq:Z_score-table}
\begin{align}
...
\begin{align}
	...
	Z &> \frac{0.1-n}{\sqrt{n}}
\end{align}
From \eqref{eq:Zscore}
\begin{align}
	\frac{0.1-n}{\sqrt{n}} &> 1.28
\end{align}
\end{enumerate}
\end{enumerate}
\newpage
