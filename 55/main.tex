...
p_A(i) &= 
	\begin{cases}
		...
		0, & \text{otherwise}
	\end{cases}\\
p_{AB}(k,i) &= \pr{B=k\left.\right | A=i}p_A(i)\\
&= 
	\begin{cases}
...
\end{align}
\begin{align}
	p_B(k) &= \sum_{i=-\infty}^{\infty} p_{AB}(k,i)\\
	&= \sum_{i=1}^5p_{AB}(k,i)\\
	\implies p_B(k) &=
	\begin{cases}
		\frac{1}{5}, & 1 \leq k \leq 5,k\neq i\\
		0, & \text{otherwise}
	\end{cases}
\end{align}
...
\begin{align}
	\Var\{X\} &= \E[X^2] - \E[X]^2\\
	&= \E[(A+B)^2] - 6^2\\
	&= \E[A^2+B^2+2AB] - \brak{2\E[A]}^2\\
	&= \E[A^2] + \E[B^2] + 2\E[AB] - 36\\
	&= \E[A^2] + \E[A^2] + 2\sum_{i=1}^5\sum^5_{k=1}ikp_{AB}(k,i) - 36\\
	&= 2\sum_{i=1}^5i^2p_A(i) + \frac{2}{20}(170) - 36\\
	&= 22 + 17 - 36\\
	&= 3
\end{align}
%\newpage
