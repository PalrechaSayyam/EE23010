...
\begin{align}
\intertext{PMF of $Z$ using $z$-transform:}
	\pr{k=Z} &= \pr{k=X-Y}\\
	\intertext{applying the z-transform on both the sides}
	z\{\pr{k=Z}\} &= z\{\pr{k=X-Y}\}\\
	\intertext{This gives us the z-transform of convolution of sequences}
	z\{p_Z(k)\} &= z\{p_X(k)*p_{-Y}(k)\}\\
	\intertext{Let $X(z)$ and $Y(-z)$ represent $z\{p_X(k)\}$ and $z\{p_{-Y}(k)\}$ respectively,}
	&= X(z)\cdot Y(-z)\\
