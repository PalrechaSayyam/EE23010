...
pmf_W = np.convolve(pmf_X, pmf_Y, mode='full')

# Calculate the actual probability of getting the same number on both dice (W = 0)
act_prob_same_no = pmf_W[5]  # Probability at W = 0
...
sim_prob_same_no = np.sum(W_simulated == 0) / num_samples

# Calculate the simulated probability of getting different numbers on both dice (|W| > 0)
sim_prob_diff_nos = np.sum(W_simulated != 0) / num_samples
...
plt.stem(k_values, pmf_W, markerfmt='o', linefmt='C0-', use_line_collection=True, label='Actual')
plt.stem(k_values,np.bincount(W_simulated + size - 1) / num_samples, markerfmt='o', linefmt='C1-', use_line_collection=True, label='Simulation')
plt.xlabel("k")
